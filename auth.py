...
def login():
    ...
    # 중복 로그인 방지: 현재 소켓에 연결된 사용자 목록(online_users, mobile_online_users) 확인 (단, MASTER는 예외)
    is_already_online = any(info.get('username') == user['username'] for info in online_users.values()) or \
                        any(info.get('username') == user['username'] for info in mobile_online_users.values())
    if is_already_online and user['role'] != 'MASTER':
